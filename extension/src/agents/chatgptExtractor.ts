...
function extractChatGptPair(): ChatGptMessagePair | null {
  ...
  for (let i = 0; i < blocks.length - 1; i++) {
    ...
    const userText =
      (userBlock.querySelector('.markdown.prose') as HTMLElement)?.innerText?.trim() ||
      userBlock.textContent?.trim();
    const assistantText =
      (assistantBlock.querySelector('.markdown.prose') as HTMLElement)?.innerText?.trim() ||
...
function sendToBackend(payload: ChatGptMessagePair) {
